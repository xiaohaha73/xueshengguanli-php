...
export default {
  ...
  async mounted () {
    ...
    // 根据账号查找信息
    const { data: res } = await this.$axios.get('/admin/getTeachers/' + id)

    if (!res.success) {
      return this.$message.error('数据异常')
    }

    const { name, account, courseid } = res.data
    this.editForm.name = name
    this.editForm.account = account
    this.editForm.courseid = courseid
  },
  data () {
    return {
      editForm: {
  ...
  methods: {
    ...
    submitForm () {
      this.$refs.editForm.validate(async (valid) => {
        ...
        const { data: res } = await this.$axios.post('/admin/updateTeacher', {
